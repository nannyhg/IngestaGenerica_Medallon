...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimActividadComercial"
SkCol        = "SKActividadComercial"
NaturalKey   = ["IdActividadComercial"]
TrackingCols = [
    "Codigo", "Descripcion", "Noteexistsflag", 
    "SubClase", "TituloSubClase", "Source"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimActividadComercial (
        SKActividadComercial   BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdActividadComercial   STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Codigo                 STRING    COMMENT 'Código identificador de la actividad comercial.',
        Descripcion            STRING    COMMENT 'Nombre o detalle descriptivo de la actividad comercial.',
        Noteexistsflag         DOUBLE    COMMENT 'Control Exactus: Bandera que indica si existen notas asociadas al registro.',
        SubClase               STRING    COMMENT 'Código o identificador de la subclase asociada a la actividad.',
        TituloSubClase         STRING    COMMENT 'Título o descripción de la subclase asociada.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Actividad Comercial — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimActividadComercial
        (SKActividadComercial, IdActividadComercial, Codigo, Descripcion, Noteexistsflag, SubClase, TituloSubClase, Source,FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            0, 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 0, 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          0, 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(Codigo, '-', Source) AS IdActividadComercial,
            Codigo,
            Descripcion,
            Noteexistsflag,
            SubClase,
            TituloSubClase,
            Source
        FROM Silver.Finanzas.ActividadComercial
    """
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimActividadComercial' )
