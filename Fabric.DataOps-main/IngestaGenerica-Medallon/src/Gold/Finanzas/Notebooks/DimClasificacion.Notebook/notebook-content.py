...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimClasificacion"
SkCol        = "SKClasificacion"
NaturalKey   = ["IdClasificacion"]
TrackingCols = [
    "Clasificacion", "Descripcion", "Agrupacion", "UsaNumerosSerie", 
    "PlantillaSerie", "AporteCodigo", "TipoCodigoBarras", 
    "UnidadMedida", "Noteexistsflag", "Source"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimClasificacion (
        SKClasificacion        BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdClasificacion        STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Clasificacion          STRING    COMMENT 'Código identificador único de la clasificación (PK).',
        Descripcion            STRING    COMMENT 'Nombre o detalle descriptivo de la clasificación.',
        Agrupacion             DOUBLE    COMMENT 'Nivel o código de agrupación jerárquica de la clasificación.',
        UsaNumerosSerie        STRING    COMMENT 'Indicador de si la clasificación utiliza números de serie (S/N).',
        PlantillaSerie         STRING    COMMENT 'Plantilla utilizada para la generación de números de serie.',
        AporteCodigo           STRING    COMMENT 'Aporte o prefijo de código asignado a esta clasificación.',
        TipoCodigoBarras       STRING    COMMENT 'Clasificación del tipo de código de barras utilizado.',
        UnidadMedida           STRING    COMMENT 'Unidad de medida estándar asociada a la clasificación.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Clasificaciones — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimClasificacion
        (SKClasificacion, IdClasificacion, Clasificacion, Descripcion, Agrupacion, UsaNumerosSerie, PlantillaSerie, AporteCodigo, TipoCodigoBarras, UnidadMedida, Noteexistsflag, Source, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            0, 'N', 'N/A', 'N/A', 'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 0, 'N', 'N/A', 'N/A', 'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          0, 'N', 'N/A', 'N/A', 'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(Clasificacion, '-', Source) AS IdClasificacion,
            Clasificacion,
            Descripcion,
            Agrupacion,
            UsaNumerosSerie,
            ...
            Noteexistsflag,
            Source
        FROM Silver.Finanzas.Clasificacion
    """
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimClasificacion' )
