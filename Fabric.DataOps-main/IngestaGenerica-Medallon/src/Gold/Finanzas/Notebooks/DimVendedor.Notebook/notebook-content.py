...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimVendedor"
SkCol        = "SKVendedor"
NaturalKey   = ["IdVendedor"]
TrackingCols = [
    "Vendedor", "Nombre", "Empleado", "Comision", "CtrComision", 
    "CtaComision", "EMail", "Activo", "Noteexistsflag", 
    "Telefono", "Source"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimVendedor (
        SKVendedor             BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdVendedor             STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Vendedor               STRING    COMMENT 'Código identificador único del vendedor (PK).',
        Nombre                 STRING    COMMENT 'Nombre completo del vendedor.',
        Empleado               STRING    COMMENT 'Código de empleado asociado al vendedor.',
        Comision               DOUBLE    COMMENT 'Porcentaje o monto de comisión asignado al vendedor.',
        CtrComision            STRING    COMMENT 'Centro de costo asociado a la comisión.',
        CtaComision            STRING    COMMENT 'Cuenta contable asociada a la comisión.',
        EMail                  STRING    COMMENT 'Correo electrónico del vendedor.',
        Activo                 STRING    COMMENT 'Indicador de si el vendedor está activo (S/N).',
        Noteexistsflag         DOUBLE    COMMENT 'Control Exactus: Bandera que indica si existen notas asociadas al registro.',
        Telefono               STRING    COMMENT 'Número de teléfono de contacto del vendedor.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Vendedores — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimVendedor
        (SKVendedor, IdVendedor, Vendedor, Nombre, Empleado, Comision, Activo, Noteexistsflag, Source, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            'N/A', 0, 'N', 0, 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 'N/A', 0, 'N', 0, 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          'N/A', 0, 'N', 0, 'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(Vendedor, '-', Source) AS IdVendedor,
            Vendedor,
            Nombre,
            Empleado,
            Comision,
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimVendedor' )
