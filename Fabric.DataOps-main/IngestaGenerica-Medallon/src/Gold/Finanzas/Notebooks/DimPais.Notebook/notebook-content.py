...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimPais"
SkCol        = "SKPais"
NaturalKey   = ["IdPais"]
TrackingCols = [
    "Pais", "Nombre", "Source"
...
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimPais (
        SKPais               BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdPais               STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Pais                 STRING    COMMENT 'Código identificador único del país (PK).',
        Nombre               STRING    COMMENT 'Nombre o detalle descriptivo del país.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Países — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimPais
        (SKPais, IdPais, Pais, Nombre, Source, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(Pais, '-', Source) AS IdPais,
            Pais, Nombre, Source
        FROM Silver.Finanzas.Pais
    """
