...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimArticulo"
SkCol        = "SKArticulo"
NaturalKey   = ["IdArticulo"]
TrackingCols = [
    "Articulo", "Descripcion", "Activo",
    "UnidadAlmacen", "UnidadEmpaque", "UnidadVenta",
    "Grupo", "Source",
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimArticulo (
        SKArticulo      BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdArticulo      STRING    COMMENT 'Identificador compuesto del artículo.',
        Articulo        STRING    COMMENT 'Código único del artículo (PK origen).',
        Descripcion     STRING    COMMENT 'Nombre completo del artículo.',
        Activo          STRING    COMMENT 'Indicador activo (S/N).',
        UnidadAlmacen   STRING    COMMENT 'Unidad de almacenamiento.',
        UnidadEmpaque   STRING    COMMENT 'Unidad de empaque.',
        UnidadVenta     STRING    COMMENT 'Unidad de venta.',
        Grupo           STRING    COMMENT 'Grupo de cuentas contables.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Artículos — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimArticulo
        (SKArticulo, IdArticulo, Articulo, Descripcion, Activo,
         UnidadAlmacen, UnidadEmpaque, UnidadVenta, Grupo, Source,FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',      'No Aplica',            'N', 'N/A', 'N/A', 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO',  'No se encontró valor', 'N', 'N/A', 'N/A', 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',    'No Definido',          'N', 'N/A', 'N/A', 'N/A', 'N/A', 'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(a.Articulo, '-', a.Source) AS IdArticulo,
            a.Articulo,
            a.Descripcion,
            a.Activo,
            a.UnidadAlmacen,
            a.UnidadEmpaque,
            a.UnidadVenta,
            ac.Descripcion AS Grupo,
            a.Source
        FROM Silver.Finanzas.Articulo a
            LEFT JOIN Silver.Finanzas.ArticuloCuenta ac
                ON a.ArticuloCuenta = ac.ArticuloCuenta AND a.Source = ac.Source
    """
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimArticulo' )
