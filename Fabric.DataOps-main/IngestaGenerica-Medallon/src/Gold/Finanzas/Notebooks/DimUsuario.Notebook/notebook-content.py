...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimUsuario"
SkCol        = "SKUsuario"
NaturalKey   = ["IdUsuario"]
TrackingCols = [
    "Usuario", "Nombre", "Tipo", "Activo", "TipoAcceso"
]

# ── DDL de creación (solo si la tabla no existe)
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimUsuario (
        SKUsuario              BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdUsuario              STRING    COMMENT 'Clave natural de la dimensión (igual a Usuario).',
        Usuario                STRING    COMMENT 'Código identificador único del usuario (PK).',
        Nombre                 STRING    COMMENT 'Nombre completo del usuario.',
        Tipo                   STRING    COMMENT 'Categoría o tipo de perfil de usuario.',
        Activo                 STRING    COMMENT 'Estado de actividad del usuario (S/N).',
        TipoAcceso             STRING    COMMENT 'Define el método de autenticación del usuario.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Usuarios — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla)
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimUsuario
        (SKUsuario, IdUsuario, Usuario, Nombre, Tipo, Activo, TipoAcceso, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            'N/A', 'N', 'N/A', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 'N/A', 'N', 'N/A', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          'N/A', 'N', 'N/A', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            ...
            Activo,
            TipoAcceso
        FROM Silver.Finanzas.UsuarioExactus
    """
