...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimMoneda"
SkCol        = "SKMoneda"
NaturalKey   = ["IdMoneda"]
TrackingCols = [
    "Moneda", "Nombre", "CodigoIso", "Noteexistsflag", "Source"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimMoneda (
        SKMoneda               BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdMoneda               STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Moneda                 STRING    COMMENT 'Código identificador único de la moneda (PK).',
        Nombre                 STRING    COMMENT 'Nombre o descripción de la moneda.',
        CodigoIso              STRING    COMMENT 'Código ISO internacional asignado a la moneda.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Monedas — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimMoneda
        (SKMoneda, IdMoneda, Moneda, Nombre, CodigoIso, Noteexistsflag, Source,FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          'N/A', 0, 'DIM_DUMMY', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(Moneda, '-', Source) AS IdMoneda,
            Moneda,
            Nombre,
            CodigoIso,
            Noteexistsflag,
            Source
        FROM Silver.Finanzas.Moneda
    """
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimMoneda' )
