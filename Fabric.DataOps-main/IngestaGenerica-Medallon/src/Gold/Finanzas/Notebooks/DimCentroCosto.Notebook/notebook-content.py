...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimCentroCosto"
SkCol        = "SKCentroCosto"
NaturalKey   = ["IdCentroCosto"]
TrackingCols = [
    "CentroCosto", "Descripcion", "AceptaDatos", "Tipo",
    "Noteexistsflag", "Source"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimCentroCosto (
        SKCentroCosto        BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdCentroCosto        STRING    COMMENT 'Identificador compuesto del centro de costo.',
        CentroCosto          STRING    COMMENT 'Código identificador único del centro de costo (PK).',
        Descripcion          STRING    COMMENT 'Nombre o detalle descriptivo del centro de costo.',
        AceptaDatos          STRING    COMMENT 'Indicador de si el centro de costo acepta movimientos directos (S/N).',
        Tipo                 STRING    COMMENT 'Clasificación o tipo de centro de costo.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Centros de Costo — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimCentroCosto
        (SKCentroCosto, IdCentroCosto, CentroCosto, Descripcion, AceptaDatos, Tipo, Noteexistsflag, Source, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'No Aplica',            'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'No se encontró valor', 'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'No Definido',          'N/A', 'N/A', 0, 'DIM_DUMMY', current_timestamp())
    """
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            concat(a.CentroCosto, '-', a.Source) AS IdCentroCosto,
            a.CentroCosto,
            a.Descripcion,
            a.AceptaDatos,
            a.Tipo,
            a.Noteexistsflag,
            a.Source
        FROM Silver.Finanzas.CentroCosto a
    """
...
try:
    ...
    #Registrar Log
    Log.LogData(result=result,data_zone='Gold', execution_artifact='DimCentroCosto' )
