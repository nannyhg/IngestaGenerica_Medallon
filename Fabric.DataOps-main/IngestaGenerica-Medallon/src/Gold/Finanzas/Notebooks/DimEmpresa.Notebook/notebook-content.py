...
# ── Tabla destino
GoldTable    = "Gold.Finanzas.DimEmpresa"
SkCol        = "SKEmpresa"
NaturalKey   = ["IdEmpresa"]
TrackingCols = [
    "Nombre", "Nit", "Pais", "ActividadComercial"
]

# ── DDL de creación (solo si la tabla no existe) 
DDLQuery = """
    CREATE TABLE IF NOT EXISTS Gold.Finanzas.DimEmpresa (
        SKEmpresa              BIGINT    COMMENT 'Clave subrogada (Surrogate Key).',
        IdEmpresa              STRING    COMMENT 'Identificador compuesto de la dimensión.',
        Nombre                 STRING    COMMENT 'Nombre o denominación asociada del conjunto.',
        Nit                    STRING    COMMENT 'Número de Identificación Tributaria del conjunto.',
        Pais                   STRING    COMMENT 'País asociado a la empresa.',
        ActividadComercial     STRING    COMMENT 'Código de la actividad comercial.',
    ...
    USING DELTA
    COMMENT 'Dimensión de Empresas simplificada — Finanzas (Gold Layer)'
"""

# ── Registros dummy (se insertan al crear la tabla) 
DummyInsert = """
    INSERT INTO Gold.Finanzas.DimEmpresa
        (SKEmpresa, IdEmpresa, Nombre, Nit, Pais, ActividadComercial, FechaCreacion)
    VALUES
        (-101, '-101', 'NO_APLICA',     'N/A', 'N/A', 'N/A', current_timestamp()),
        (-201, '-201', 'NO_ENCONTRADO', 'N/A', 'N/A', 'N/A', current_timestamp()),
        (-301, '-301', 'NO_DEFINIDO',   'N/A', 'N/A', 'N/A', current_timestamp())
"""
...
def ReadSilverAndTransform():
    SourceQuery = """
        SELECT
            UPPER(TRIM(Conjunto))       AS IdEmpresa,
            UPPER(REPLACE(REPLACE(LTRIM(RTRIM(Nombre)),'  ', ' '),'  ', ' ')) AS Nombre, 
            Nit, 
            UPPER(TRIM(Pais))           AS Pais, 
            ActividadComercial
        FROM Silver.Finanzas.EmpresasExactus
    """
